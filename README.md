...
3. `cp config/lti_dev_key_sample.json config/lti_dev_key.json`
4. `mvim config/lti_dev_key.json`
   Replace all occurrences of `{ccm_app_url}` with the hostname copied in the earlier step.
5. Go to Canvas "Developer Keys" management page at (https://umich.instructure.com/accounts/1/developer_keys).  Click the button for "+ Developer Key", then "+ LTI Key".
6. When the "Key Settings" modal appears, select "Paste JSON" from the "Method" menu in the "Configure" section.  Then copy the contents of your `config/lti_dev_key.json` and paste it into the "LTI 1.3 Configuration" field.
7. Save LTI Key.
8. Edit LTI Key, use "Manual Entry" under the "Method" menu to access "LTI Advantage Services" under the "Configuration" section.  Enable all options, then click the "Save" button.
...
10. Add the app to a course.  Go to (https://umich.instructure.com/courses/nnnnnn/settings/configurations), where "nnnnnn" is the course ID.
